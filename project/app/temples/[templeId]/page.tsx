...
export default function TemplePage({ params }: { params: { templeId: string } }) {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      setTemple({
        ...
        rating: 4.8,
        reviews: 2500,
        placeId: "1", // Reference to Varanasi
        placeName: "Varanasi",
        significance:
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <div className="mb-6">
        <div className="flex flex-wrap items-center gap-2">
          <Link href="/temples" className="inline-flex items-center text-amber-600 hover:text-amber-700">
            <ChevronLeft className="h-4 w-4 mr-1" />
            All Temples
          </Link>
          <span className="text-gray-400">•</span>
          <Link href={`/places/${temple.placeId}`} className="text-amber-600 hover:text-amber-700">
            {temple.placeName}
          </Link>
        </div>
      </div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-lg shadow-md overflow-hidden">
            <div className="relative h-80">
              <Image
                src={temple.images[0] || "/placeholder.svg"}
                alt={temple.name}
                fill
              ...
              <Tabs defaultValue="about" className="w-full">
                ...
                <TabsContent value="map" className="pt-4">
                  <div className="h-80 rounded-lg overflow-hidden">
                    <PlaceMap coordinates={temple.coordinates} name={temple.name} />
                  </div>
                </TabsContent>
              </Tabs>
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardContent className="p-6">
              ...
              <Link href={`/places/${temple.placeId}`}>
      ...
      <div className="mt-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {relatedTemples.map((relatedTemple) => (
            <Link key={relatedTemple.id} href={`/temples/${relatedTemple.id}`}>
              <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
                <div className="relative h-48">
                  <Image
                    src={relatedTemple.image || "/placeholder.svg"}
